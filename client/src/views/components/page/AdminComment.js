...
import ItemCart from '../../../components/modal/ItemCart';
export default function AdminComment() {
    ...
    return (
        <Container>
            <Row>
                <Col xs="4" style={{overflow:"auto",maxHeight:'calc(100vh - 65px)'}}>
                    <Card style={{ width: '100%' }} border="light">
                        {products.map(product => (
                            <Row className='d-flex align-items-center' style={{ borderBottom: '1px solid #fbc7d4',background:productId===product._id&&'#9796f0'||'' }} onClick={() => setStateProductId({ productId: product._id })}>
                                <Col xs="3">
                                    <Card.Img variant="top" src={product.imgCarUrl} />
                                </Col>
                                <Col xs="9" style={{ padding: '0px' }}>
                                    <Card.Body>
                                        <Card.Title as="h7">{product.nameCar}</Card.Title>
                                    </Card.Body>
                                </Col>
                            </Row>
                        ))}
